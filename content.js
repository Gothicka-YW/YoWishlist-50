...
(function(){
  ...
  function pickTargetSection(scope, container){ const {wish,sale,wishHead,saleHead}=findSectionContainers(); if(scope==='wish'&&wish) return {root:wish, head:wishHead, name:'wish'}; if(scope==='sale'&&sale) return {root:sale, head:saleHead, name:'sale'}; return sectionRootFor(container); }
...
const docTop = rect.top + window.scrollY;
  ...
  function startPicker(){ if(state.picking) return; state.picking=true; const overlay=document.createElement('div'); Object.assign(overlay.style,{position:'fixed',inset:'0',background:'rgba(0,0,0,0.08)',zIndex:2147483646,cursor:'crosshair',pointerEvents:'none'}); const tip=document.createElement('div'); tip.textContent='Click a single tile (outer white area). Press Esc to cancel.'; Object.assign(tip.style,{position:'fixed',left:'50%',transform:'translateX(-50%)',top:'10px',background:'#111',color:'#fff',padding:'6px 10px',borderRadius:'8px',fontSize:'12px',zIndex:2147483647}); document.documentElement.append(overlay,tip); toast('Picker armed — click a tile');
    ...
    function onDocClick(e){ e.preventDefault(); e.stopPropagation(); const target=document.elementFromPoint(e.clientX,e.clientY); let node=target; while(node && node!==document.body && (!node.classList || node.classList.length===0)){ node=node.parentElement; } const card=closestCard(node)||node; const cardSelector=tagAndClasses(card); let chosenContainer=null, chosenCount=0; let cur=card.parentElement; for(let hops=0;cur && hops<12;hops++,cur=cur.parentElement){ try{ const count=Array.from(cur.querySelectorAll(cardSelector)).filter(n=>n.offsetParent!==null).length; if(count>=6){ chosenContainer=cur; chosenCount=count; break; } if(!chosenContainer && count>=3){ chosenContainer=cur; chosenCount=count; } }catch{} } if(!chosenContainer) chosenContainer=document; state.containerSel=uniqueCssPath(chosenContainer); state.cardSel=cardSelector; state.selectorHint=uniqueCssPath(card); chrome.storage.sync.set({ yl50_container: state.containerSel, yl50_card: state.cardSel, yl50_selectors: state.selectorHint }); cleanup(); toast(`Saved ✓ Found ${chosenCount} tiles in section`); }
